...
namespace WebServicos.Models
{
    ...
    /// <summary>
    /// Notificação enviada para um utilizador sobre alterações de pedidos ou novas mensagens.
    /// Permite rastrear quais notificações foram lidas pelo utilizador.
    /// </summary>
    public class Notificacao
    {
        /// <summary>Identificador único da notificação (chave primária).</summary>
        [Key]
        public int Id { get; set; }

        /// <summary>Chave estrangeira para o utilizador destinatário da notificação.</summary>
        [Required]
        public string UtilizadorId { get; set; } = string.Empty;

        /// <summary>Utilizador que recebe a notificação (navegação).</summary>
        [ForeignKey(nameof(UtilizadorId))]
        [Display(Name = "Utilizador")]
        public ApplicationUser? Utilizador { get; set; }

        /// <summary>Tipo de notificação (ex: "PedidoAlterado", "NovaMensagem", "NovoPedido").</summary>
        [Required]
        [StringLength(50)]
        [Display(Name = "Tipo")]
        public string Tipo { get; set; } = string.Empty;

        /// <summary>Título/resumo da notificação apresentado ao utilizador.</summary>
        [Required]
        [StringLength(200)]
        [Display(Name = "Título")]
        public string Titulo { get; set; } = string.Empty;

        /// <summary>Descrição completa da notificação.</summary>
        [Required]
        [StringLength(500)]
        [Display(Name = "Descrição")]
        public string Descricao { get; set; } = string.Empty;

        /// <summary>ID do pedido relacionado (opcional, para notificações de pedidos).</summary>
        [Display(Name = "Pedido")]
        public int? PedidoId { get; set; }

        /// <summary>Data e hora UTC em que a notificação foi criada.</summary>
        [Display(Name = "Data/Hora")]
        public DateTime DataHora { get; set; } = DateTime.UtcNow;

        /// <summary>Indica se a notificação foi lida pelo utilizador.</summary>
        [Display(Name = "Lida")]
        public bool Lida { get; set; } = false;

        /// <summary>Data e hora UTC em que a notificação foi marcada como lida (null = ainda não lida).</summary>
