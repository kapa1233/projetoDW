    ...
    public class CreateModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                // ✨ Notificar cliente sobre seu próprio pedido
                await _notificacaoService.CriarNotificacaoAsync(
                    utilizadorId: user.Id,
                    tipo: "PedidoCriado",
                    titulo: "Pedido submetido com sucesso",
                    descricao: $"O seu pedido '{Pedido.TituloProjeto}' foi submetido e está aguardando análise.",
                    pedidoId: Pedido.Id
                );
...
// ✨ Notificar administradores sobre novo pedido
                try
                {
                    // Use the injected UserManager to safely retrieve users by role
                    ...
                    foreach (var admin in admins)
                    {
                        await _notificacaoService.CriarNotificacaoAsync(
                            ...
                            tipo: "NovoPedido",
                            titulo: "Novo pedido submetido",
                            descricao: $"Novo pedido de {Pedido.TituloProjeto} - Clique para rever.",
                            pedidoId: Pedido.Id
                        );
                    }
                }
                ...
                {
                    _logger.LogError(ex, "Erro ao notificar administradores");
                }
            }
            catch (Exception ex)
            {
