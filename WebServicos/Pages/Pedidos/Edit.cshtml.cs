...
namespace WebServicos.Pages.Pedidos
{
    ...
    public class EditModel : PageModel
    {
        ...
        public EditModel(ApplicationDbContext db, UserManager<ApplicationUser> um, ILogger<EditModel> logger, IHubContext<NotificacoesHub> hubContext, INotificacaoService notificacaoService)
        {
            _db = db;
            _userManager = um;
            _logger = logger;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // Armazena o estado anterior para verificar se houve mudança
                    ...
                    // Se o estado foi alterado, envia notificação ao cliente
                    if (estadoAnterior != Pedido.Estado)
                    {
                        _logger.LogInformation("Estado do pedido #{Id} alterado de {EstadoAnterior} para {EstadoNovo}. Notificando cliente {ClienteId}.", 
                            Pedido.Id, estadoAnterior, Pedido.Estado, pedidoExistente.ClienteId);
                        
                        // ✨ Notificar cliente via serviço de notificações (BD + SignalR)
                        var estadoNome = Pedido.Estado.GetDisplayName();
                        await _notificacaoService.NotificarAtualizacaoPedidoAsync(
                            pedidoExistente.Id,
                            $"Pedido atualizado: {estadoNome}",
                            $"O seu pedido '{pedidoExistente.TituloProjeto}' está agora em: {estadoNome}"
                        );
                        
                        // Captura valores para o Task.Run (evita problemas com serviços scoped em background)
                        var clienteIdBg = pedidoExistente.ClienteId;
                        var tituloBg = pedidoExistente.TituloProjeto;
                        var estadoBg = Pedido.Estado.GetDisplayName();
                        var pedidoIdBg = Pedido.Id;
                        ...
                        _ = Task.Run(async () =>
                        {
                            try
                            {
                                await _hubContext.Clients.User(clienteIdBg).SendAsync("PedidoAtualizado",
                                    tituloBg, estadoBg, DateTime.UtcNow.ToString("HH:mm"));
                            }
                            catch (Exception ex)
                            {
